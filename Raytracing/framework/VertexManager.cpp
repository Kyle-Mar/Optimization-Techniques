...
VertexManager::VertexManager(VulkanContext* ctx_, const std::vector<VertexManager::Input>& inputs_)
{
    ...
    if( inputs.size() < 1 ){
        throw std::runtime_error("Must have at least one vertex input");
    }

    for(unsigned i=0;i<this->inputs.size();++i){
        std::string name = inputs[i].name;
        if( this->nameToIndex.contains(name) )
            throw std::runtime_error("Duplicate input: "+name);
        if( name == "indices" )
            throw std::runtime_error("'indices' is not a valid name");
...
VertexManager::Info VertexManager::addIndexedDataHelper(std::vector<AttribInfo>& V)
{
    ...
    for(auto& d : V ){
        if( supplied.find(d.name) != supplied.end() ){
            throw std::runtime_error("Duplicate name: "+d.name);
        }
    ...
    if(!missing.empty()){
        auto tmp = join(", ",extras);
        throw std::runtime_error("Missing: "+tmp);
    }
    ...
    for(AttribInfo& ai : V ){
        ...
        if(ai.name == "indices")
            ...
            if(nv == -1)
                ...
                if( nv != tmp ){
                    throw std::runtime_error("Vertex element count mismatch");
                }
            }
        }
    }
...
int VertexManager::addDataForName(AttribInfo& ai)
{
    if( !this->nameToIndex.contains(ai.name) ){
        throw std::runtime_error("Unexpected name: "+ai.name);
    }
    ...
    if( ai.name == "indices" ){
        if( ai.format != VK_FORMAT_R32_UINT )
            throw std::runtime_error("Bad format for "+ai.name);
    } else {
...
{
    if(this->pushedToGPU)
        throw std::runtime_error("pushToGPU called twice");
...
{
    if(! this->pushedToGPU )
        throw std::runtime_error("Need to pushToGPU() first");
