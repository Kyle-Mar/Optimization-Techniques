...
{
    if(_inCompute ){
        throw std::runtime_error("beginCompute() called twice with no intervening endCompute()");
    }
    if(_inFrame){
        throw std::runtime_error("beginCompute() called while still rendering a frame");
    }
    _inCompute=true;
...
{
    if(_inFrame ){
        throw std::runtime_error("beginFrame() called twice with no intervening endFrame() or endCompute()");
    }
    if(_inCompute){
        throw std::runtime_error("beginFrame() called while still in a compute operation");
    }
    _inFrame=true;
    doBegin(ctx,true);
}
...
{
    if(!_inCompute ){
        throw std::runtime_error("endCompute called without matching beginCompute");
    }
