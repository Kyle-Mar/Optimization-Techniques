...
class Wave{
  public:
    ...
    std::uint32_t numSamples;   //calculated
    ...
    Wave(std::string filename){
        ...
        while(true){
            ...
            if( 0 == std::memcmp(chunkID, "fmt ",4) )
                ...
                dataStartsAt = unsigned(in.tellg())-4;
                readDataChunk(in);
            }
            else{
                if( dataStartsAt != 0 ){
                    std::cout << "CHUNK AFTER DATA\n";
                }
                readUnknownChunk(in);
            }
        ...
        in.clear();
        in.seekg(0);
        headerData.resize(dataStartsAt);
        in.read( (char*) headerData.data(), headerData.size() );
    }
    ...
    */
    void write(std::string filename){
        ...
        uint32_t chunkSize = numFrames*format.bytesPerFrame;
  ...
  private:
    ...
    void readDataChunk(std::ifstream& in){
        ...
        numSamples = numFrames * format.numChannels;
        if( in.fail() )
