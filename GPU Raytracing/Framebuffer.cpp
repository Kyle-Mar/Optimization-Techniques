    ...
    VkFormat format_, std::string name_)
{
    ...
    VkImageLayout finalLayoutC;
    VkImageLayout finalLayoutD;
...
//~ void Framebuffer::clear(
        ...
        //~ std::optional<int> clearStencilValue
//~ ){
    //~ /**
    //~ """
    //~ Clear the color, depth, and/or stencil buffers associated with this Framebuffer
        //~ @param cmd The command buffer
        //~ @param clearColorValue The color to use for the clear; if None, do not clear the color channels.
        //~ @param clearDepthValue The depth value to use for the clear; if None, do not clear depth buffer.
        //~ @param clearStencilValue The stencil value to use for the clear; if None, do not clear stencil buffer.
        //~ @param r,g,b,a The clear color. Only used if clearColorValue is not provided
    //~ """
    //~ */
...
void Framebuffer::beginRenderPassHelper(
    ...
    if(layerIndex == -1){
        ...
        if(layerIndex < 0 or layerIndex >= (int)this->numLayers){
            if( this->numLayers == 1 ){
                throw std::runtime_error("Bad layer (" + str(layerIndex)+
    ...
    if( layerIndex == -1 ){
        ...
        if(loadOp == VK_ATTACHMENT_LOAD_OP_DONT_CARE)
            ...
            throw std::runtime_error("Internal error: Bad loadOp");
    } else {
        if(loadOp == VK_ATTACHMENT_LOAD_OP_DONT_CARE)
