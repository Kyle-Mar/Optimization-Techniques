...
Image* ImageManager::loadLayers(std::vector<std::string> filenames, std::string name)
{
    ...
    */
    if(!ctx)
        throw std::runtime_error("ImageManager.initialize() has not been called");
    ...
    if( _imgmap.contains(name)){
        ...
        if( it->format != VK_FORMAT_R8G8B8A8_UNORM ||
            it->viewType != VK_IMAGE_VIEW_TYPE_2D_ARRAY){
                throw std::runtime_error("Two images have the same name but different formats");
        }
        return it;
    }
    ...
    for(unsigned fi=0;fi<filenames.size();++fi){
        ...
        if(width == -1 ){
            ...
            if( w != width or h != height ){
                throw std::runtime_error("Image size mismatch: "+filename+" is "+str(w)+"x"+str(h)+" but "+
                    filenames[0]+" is "+str(width)+"x"+str(height) );
            }
        }
        if(fmt != "RGBA8"){
            throw std::runtime_error(filename+": Expected RGBA8 image, but it is "+str(fmt));
        }
...
Image* ImageManager::load16(std::string filename)
{
    ...
    if( std::get<2>(tmp) != "RGBA16" ){
        throw std::runtime_error("Bad image format: Got "+std::get<2>(tmp)+" but expected RGBA16");
    }
