...
Image::Mip::Mip(int w, int h, const std::span<const char>& pixels_){
    assert(w>0);
    assert(w<32768);
    assert(h>0);
    assert(h<32768);
...
Image::Layer::Layer(int w, int h, const std::span<const char>& pixels, int numMips_){
    assert(w>0);
    assert(w<32768);
    assert(h>0);
    assert(h<32768);
...
Image::Image(
    ...
    if( pixels.empty() ){
        for(unsigned i=0;i<numLayers;++i){
            this->layers.push_back(Layer(width,height,std::span<const char>(),numMips_));
        }
    } else {
        assert( numLayers == (unsigned)pixels.size() );
        for(std::span<const char> pix : pixels){
            this->layers.push_back( Layer( width,height,pix,numMips_ ) );
        }
    }
