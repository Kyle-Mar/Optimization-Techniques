...
class VertexManager{
  public:
    ...
    //~ class IDItem{
      //~ public:
        //~ typedef std::variant<
            //~ std::vector<float>,
            //~ std::vector<math2801::vec2>,
            //~ std::vector<math2801::vec3>,
            //~ std::vector<math2801::vec4>,
            //~ std::span<float>,
            //~ std::span<math2801::vec2>,
            //~ std::span<math2801::vec3>,
            //~ std::span<math2801::vec4>
            //~ > Data;

        //~ IDItem(std::string name_, Data& data_){
            //~ this->name=name_;
            //~ this->data=data_;
        //~ }
        //~ std::string name;
    ...
    /// @param args The data to add. Each item is passed as a pair:
    //          The name and the data. Data must be a vector or span.
    /// @return An Info structure describing the newly added data.
    //~ template<typename ...U>
    //~ VertexManager::Info addIndexedData(const std::initializer_list<IDItem>& items);
  ...
  private:
    ...
    Info addIndexedDataHelper(std::vector<AttribInfo>& V, const std::string& name, const std::vector<T>& data, U... args)
    {
        addAttribInfo(V,name,data);
