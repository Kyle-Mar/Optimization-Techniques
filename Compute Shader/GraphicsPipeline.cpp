...
//~ GraphicsPipeline::GraphicsPipeline( VulkanContext* ctx_, std::string name_,
        //~ PipelineLayout* pipelineLayout_,
        //~ const std::vector<Framebuffer*>& framebuffers_,
        //~ const std::initializer_list<PipelineOption>& args) : Pipeline(
                                            //~ ctx_,pipelineLayout_,
                                            //~ VK_PIPELINE_BIND_POINT_GRAPHICS,name_)
//~ {
    //~ this->_setDefaults();
...
void GraphicsPipeline::finishInit()
{
    ...
    for(Framebuffer* fb : this->framebuffers){
        for(RenderPass* renderPass : fb->getAllRenderPasses()){
            ...
            VkPipelineViewportStateCreateInfo vstate {
                ...
                .pNext=nullptr,
                .flags=0,
                .viewportCount = (unsigned)renderPass->numLayers,
                .pViewports = vport.data(),
                .scissorCount = (unsigned)renderPass->numLayers,
            ...
            this->pipelines[std::make_pair(fb,renderPass)] = pipeline;
        } //end for renderpass
    }//end for framebuffer
}
